...
function isTriangle(a, b, c) {
  ...
  if ((c === max && a + b > c) || (a === max && b + c > a) || (b === max && a + c > b)) {
    res = true;
  } else {
    res = false;
  }
...
function isInsideCircle(circle, point) {
  const xDif = Math.abs(circle.center.x - point.x);
  const yDif = Math.abs(circle.center.y - point.y);
  return (circle.radius > (xDif ** 2 + yDif ** 2) ** 0.5);
}
    ...
    for (let i = 0; i < str.length; i += 1) {
      if (i % 2 !== 0) {
        ...
        sum += (parseInt(str[i], 10) * 2) > 9
          ? (parseInt(str[i], 10) * 2) - 9 : (parseInt(str[i], 10) * 2);
      }
    }
  } else {
    for (let i = 0; i < str.length; i += 1) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (openBrackets.indexOf(str[i]) !== -1) {
      ...
      if (stack.length === 0) {
        res = false;
        break;
      } else if (stack[stack.length - 1] === openBrackets[closeBrackets.indexOf(str[i])]) {
        stack.pop();
      } else {
        res = false;
        break;
...
function getCommonDirectoryPath(pathes) {
  ...
  let path = '';
  let r = 0;
  for (let n = 0; n < pathes.length; n += 1) {
    pathesArr.push(pathes[n].split('/'));
  }
